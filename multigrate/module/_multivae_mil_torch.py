...
class MultiVAETorch_MIL(BaseModuleClass):
    def __init__(
        ...
        for _ in range(len(self.ord_idx) + len(self.reg_idx)): # one head per standard regression and one per ordinal regression
    ...
    def inference(self, x, cat_covs, cont_covs):
        ...
        if self.hierarchical_attn:
            ...
            predictions.extend([classifier(aggr_bag_level) for classifier in self.classifiers]) # each one num of bags in batch x num of classes
            predictions.extend([regressor(aggr_bag_level) for regressor in self.regressors])
        else:
        ...
        inference_outputs.update({'predictions': predictions}) # predictions are a list as they can have different number of classes
    ...
    def loss(self,
        ...
        for i in range(len(self.class_idx)): 
            classification_loss += F.cross_entropy(predictions[i], classification[:, i].long()) # assume same in the batch
            accuracies.append(torch.sum(torch.eq(torch.argmax(predictions[i], dim=-1), classification[:, i])) / classification[:, i].shape[0])
        ...
        for i in range(len(self.ord_idx)):
            regression_loss += F.mse_loss(predictions[len(self.class_idx) + i].squeeze(), ordinal_regression[:, i])
        for i in range(len(self.reg_idx)):
            regression_loss += F.mse_loss(predictions[len(self.class_idx) + len(self.ord_idx) + i].squeeze(), regression[:, i])
        ...
        return LossRecorder(loss, reconst_losses, self.vae.loss_coefs['kl'] * kl_loss, kl_global=torch.tensor(0.0), integ_loss=integ_loss, cycle_loss=cycle_loss, class_loss=classification_loss, accuracy=accuracy, reg_loss=reg_loss, regression_loss=regression_loss)
