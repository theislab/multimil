...
def validate_params(recon_coef,
                    ...
                    adversarial,
                    pair_split):
    # cycle loss and cross loss should not be used together
    if cross_coef > 0 and cycle_coef > 0:
        return False
...
def create_config(base_config,
    ...
    model_params['cross_coef'] = cross_coef
...
def parse_args():
    ...
    parser.add_argument('--cross-coefs', nargs='+', type=float, required=True)
    ...
    parser.add_argument('--ae-optimize-every', type=int, required=True)
